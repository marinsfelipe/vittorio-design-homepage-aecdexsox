routerAdd('GET', '/backend/v1/public/images', (e) => {
  ...
  const category = e.request.url.query().get('category')
  let filter = ''
  if (category) {
  ...
  try {
    const records = filter
      ? $app.findRecordsByFilter('images', filter, '-created', 100, 0)
      : $app.findRecordsByFilter('images', '', '-created', 100, 0)
